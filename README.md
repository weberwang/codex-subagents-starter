...
一个用于演示和复用 Codex 子代理配置的最小模板仓库。

## 项目目标

这个仓库不承载业务代码，只提供一组边界清晰的 Codex 子代理模板，方便你复制到真实项目中继续扩展。
...
2. 根据项目实际需要调整 `developer_instructions`。
3. 在复制后的项目中保留一份类似的校验脚本，避免模板被误改坏。
...
这个命令会扫描 `.codex/agents/*.toml`，检查每个代理模板是否包含以下核心字段：
